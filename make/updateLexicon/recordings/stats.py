...
def processing_corpus(corpus_path):
    ...
    for f in os.listdir(corpus_path):
        ...
        person = re.sub(r'\s*[0-9]+\s*', '', file_name.replace('.txt', '')).strip()
...
def write_to_xlsx(output_name, origin, total_frequency, frequencies):
    print(output_name)
    workbook = xlsxwriter.Workbook('stats/{}_recordings_stats_{}.xlsx'.format(output_name, time.strftime("%Y-%m-%d_%H:%M")))
